...
Система состоит из трёх микросервисов и инфраструктурных компонентов:
- HTTP Service: универсальный FastAPI‑сервис для приёма событий/запросов, аудита и интеграций. БД: `request_log`, `request_audit`, `proxy_audit`.
- TG Bot Service: сервис Telegram‑бота (aiogram). Хранит профиль пользователя, входящие/исходящие сообщения, настройки приветствия. БД: `user_state`, `bot_settings`, `user_profile`, `incoming_message`, `outgoing_message`.
- AI Service: генерация ответов через локальную LLM (Ollama). БД: `ai_session`, `ai_message`.
- Инфраструктура: PostgreSQL, PgAdmin, ngrok (туннель), Ollama (модель по умолчанию `mistral`).
...
2. Интеграция Telegram‑бота (приветствие, первый контакт, журналирование)
3. Интеграция AI‑ответов через Ollama
4. Наблюдаемость, логирование и трассировка
...
- User Story: Как пользователь бота, я хочу получать AI‑ответ на мои сообщения, чтобы продолжать диалог.
  - Критерии приёмки:
    ...
    - При недоступности AI возвращается graceful fallback вида `AI: <prompt>` или молчание.
...
4. Генерация AI‑ответа
   ...
   - Поток: `POST AI /generate` → `ai_session` ensure → запись `ai_message(user)` → вызов Ollama → запись `ai_message(assistant)` → возврат ответа боту.
...
- Контракты API:
  ...
  - AI Service: `POST /generate` → `{reply: string}`.
...
- Временные лимиты: HTTP‑клиенты используют таймауты 3–30 секунд.

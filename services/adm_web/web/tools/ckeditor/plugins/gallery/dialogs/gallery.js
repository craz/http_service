...
CKEDITOR.dialog.add('galleryDialog', function (editor) {
    ...
    function getImagesContainerBlock(dialog, dom) {
        ...
        var obj = dom.$.querySelector(".content_gallery__slider-container");

        obj = obj.querySelector(".content_gallery__slider");
        obj = obj.querySelector(".swiper-wrapper");
    ...
    function createDOMViewerRun(dialog) {
        ...
        var ulObjCont = galleryDOM.append('div');
        ulObjCont.setAttribute('id', id + '_images');
        ulObjCont.setAttribute('class', 'content_gallery__slider-container');

        var ulObj = ulObjCont.append('div');
        ulObj.setAttribute('class', 'content_gallery__slider');

        var swiperWrapperDiv = ulObj.append("div");
        swiperWrapperDiv.setAttribute('class', 'swiper-wrapper');

        // ulObj.setAttribute('style', 'display:none');

        feedUlWithImages(dialog, swiperWrapperDiv);

        var swiperTool1 = ulObjCont.append("div");
        swiperTool1.setAttribute('class', 'swiper-btn swiper-btn-next');
        var swiperTool2 = ulObjCont.append("div");
        swiperTool2.setAttribute('class', 'swiper-btn swiper-btn-prev');
        var swiperTool3 = ulObjCont.append("div");
        swiperTool3.setAttribute('class', 'swiper-pagination');

        return galleryDOM;
    }

    function feedUlWithImages(dialog, ulObj) {
        var i, liObj, newImgDOM, pictureDomObj;
        for (i = 0; i < dialog.imagesList.length; i += 1) {
            liObj = ulObj.append('div');
            liObj.setAttribute('contenteditable', 'false');
            liObj.setAttribute('class', 'swiper-slide');

            pictureDomObj = liObj.append('picture');

            newImgDOM = pictureDomObj.append('img');
            newImgDOM.setAttribute('src', removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL]));
            newImgDOM.setAttribute('title', '');
            newImgDOM.setAttribute('alt', '');
            newImgDOM.setAttribute('contenteditable', 'false');

            newImgDOM = pictureDomObj.append('source');
            newImgDOM.setAttribute('srcset', removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL]));
            newImgDOM.setAttribute('media', '(max-width: 991px)');
            newImgDOM.setAttribute('contenteditable', 'false');

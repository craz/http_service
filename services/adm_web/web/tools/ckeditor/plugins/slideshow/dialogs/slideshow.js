...
//				' src="' + 'data:text/html;charset=utf-8,' +  strVar + '"' +
	...
	function initImgListFromDOM(dialog, slideShowContainer) {
		...
		var arr = slideShowContainer.$.getElementsByClassName("data-div");
		console.log(arr);

		combo = dialog.getContentElement( 'slideshowinfoid', 'imglistitemsid');
		for (i = 0; i < arr.length; i++) {
			image = arr[i];
			src = image.getAttribute('data-href');
	...
	function getImagesContainerBlock2(dialog, dom) {
		var obj = dom.getElementsByTag("div");
		if (obj == null) return null;
		obj = obj.getItem(0);
		if (obj == null) return null;

		obj = obj.getElementsByTag("div");
		if (obj == null) return null;
...
//		str += "});";
	...
	function feedUlWithImages2(dialog, imgObj) {
		for ( var i = 1; i < dialog.imagesList.length  ; i++ ) {

			var idiv = imgObj.append('div');
			idiv.setAttribute( 'class', 'gallery__item');
			idiv.setAttribute( 'contenteditable', 'false');

			if (i < 4) {
				var iimg = idiv.append('div');
				iimg.setAttribute( 'data-href', removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL]) );
				iimg.setAttribute('class', 'gallery__link data-div');
				iimg.setAttribute('data-fancybox', 'gallery');
				iimg.setAttribute('style', "background-image: url('"+removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL])+"')");
				iimg.setAttribute( 'contenteditable', 'false' );
			} else if (i == 4) {
				
				var iimg = idiv.append('div');
				iimg.setAttribute( 'data-href', removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL]) );
				iimg.setAttribute('class', 'gallery__link gallery__link--more data-div');
				iimg.setAttribute('data-fancybox', 'gallery');
				iimg.setAttribute('style', "background-image: url('"+removeDomainFromUrl(dialog.imagesList[i][IMG_PARAM.URL])+"')");
				iimg.setAttribute( 'contenteditable', 'false' );

				var ispan = iimg.append('div');

				ispan.setAttribute( 'contenteditable', 'false' );
				ispan.setAttribute('class', 'gallery__count data-div');
				ispan.appendText('+'+(dialog.imagesList.length-4));

			} if (i > 4) {
				iimg.setAttribute( 'style', 'display:none' );
			}
		}
	};
	...
	function createDOMdGalleryRun3(dialog) {

		var slideshowDOM = editor.document.createElement( 'div' );
		slideshowDOM.setAttribute( 'class', 'slideshowPlugin');
		slideshowDOM.setAttribute( 'contenteditable', 'false');

		var domObj = slideshowDOM.append('div');
		domObj.setAttribute('class', 'block_slider_inner_all');
		domObj.setAttribute( 'contenteditable', 'false');
				
		return slideshowDOM;
	};

	function createDOMdGalleryRun2(dialog) {
		...
		var domObj = slideshowDOM.append('div');
		domObj.setAttribute('class', 'gallery');
		domObj.setAttribute( 'contenteditable', 'false');

		var imgObj = domObj.append('div');
		imgObj.setAttribute('class', 'gallery__list');
		imgObj.setAttribute( 'contenteditable', 'false');

		var imgObj1 = imgObj.append('div');
		imgObj1.setAttribute('class', 'gallery__item gallery__item--lg');
		imgObj1.setAttribute( 'contenteditable', 'false');

		var imgObj2 = imgObj.append('div');
		imgObj2.setAttribute('class', 'gallery__list gallery__list--inner');
		imgObj2.setAttribute( 'contenteditable', 'false');


		var iimg = imgObj1.append('div');
		iimg.setAttribute( 'data-href', removeDomainFromUrl(dialog.imagesList[0][IMG_PARAM.URL]) );
		iimg.setAttribute('class', 'gallery__link data-div');
		iimg.setAttribute('data-fancybox', 'gallery');
		iimg.setAttribute('style', "background-image: url('"+removeDomainFromUrl(dialog.imagesList[0][IMG_PARAM.URL])+"')");
		iimg.setAttribute( 'contenteditable', 'false' );

		feedUlWithImages2(dialog, imgObj2);

		/*var a1 = domObj.append('div');
		a1.setAttribute('class', 'block_slider_inner_arrow_left');
		a1.setAttribute( 'contenteditable', 'false');

		var a2 = domObj.append('div');
		a2.setAttribute('class', 'block_slider_inner_arrow_right');
		a2.setAttribute( 'contenteditable', 'false');*/
	...
	function ClickOkBtn(dialog) {
		...
		// Create a new DOM
	    var slideshowDOM = createDOMdGalleryRun2(dialog);
		...
		extraStyles.width = '70px';
		extraStyles.height = '70px';
...
//				console.log( "Images New -> " + this.imagesList );
				...
				var slideShowContainer =  getImagesContainerBlock2(this, slideshowDOM);
